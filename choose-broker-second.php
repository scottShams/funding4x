<?php
    ...
    $stmt = $pdo->prepare("SELECT status FROM mt5_details WHERE user_id = ?");
    $stmt->execute([$user['id']]);

    $mt5 = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($mt5) {
        // Row exists in mt5_details
        if ($mt5['status'] !== 'pass') {
            header("Location: choose-broker.php");
            exit;
        }
    } else {
        // No MT5 details found → force user to choose broker
        header("Location: choose-broker.php");
        exit;
    }

    // Check if MT5 details second are already submitted
    $stmt = $pdo->prepare("SELECT id FROM mt5_details_second WHERE user_id = ?");
    $stmt->execute([$user['id']]);
    $hasMT5DetailsSecond = $stmt->fetch(PDO::FETCH_ASSOC) ? true : false;
?>
...
<body class="min-h-screen flex flex-col">
    ...
    <div id="already-submitted-modal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 <?php echo $hasMT5DetailsSecond ? '' : 'hidden'; ?> animate-fade-in">
        <div class="bg-white p-10 rounded-3xl shadow-2xl max-w-lg mx-4 border-t-4 border-primary-purple transform scale-95 animate-modal-appear">
            <div class="text-center">
                ...
                <p class="text-gray-700 mb-8 leading-relaxed text-lg">
                    Hello <?php echo htmlspecialchars($user['name']); ?>, you have already submitted your Trading Test 2 MT5 account details for monitoring.
    ...
    <script>
        function handleFormSubmit(event) {
            ...
            // Send to server
            fetch('store_mt5_details_second.php', {
                method: 'POST',
                headers: {

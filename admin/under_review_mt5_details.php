<?php
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_status') {
        ...
        $table = $_POST['table'] ?? 'mt5_details'; // default to first table
        ...
        // Validate table
        if (!in_array($table, ['mt5_details', 'mt5_details_second'])) {
        ...
        $userIdStmt = $pdo->prepare("SELECT user_id FROM $table WHERE id = ?");
        ...
        if ($newStatus === 'running') {
            // Get user_id from the table
            $userStmt = $pdo->prepare("SELECT user_id FROM $table WHERE id = ?");
        ...
        if ($newStatus === 'fail') {
            ...
            $stmt = $pdo->prepare("UPDATE $table SET status = ?, fail_reason = ?, status_updated_at = NOW() WHERE id = ?");
            $success = $stmt->execute([$newStatus, $failReasonJson, $mt5Id]);
        } else {
            $stmt = $pdo->prepare("UPDATE $table SET status = ?, status_updated_at = NOW() WHERE id = ?");
        ...
        if ($success) {
            ...
            if ($newStatus === 'running') {
                ...
                $userStmt = $pdo->prepare("SELECT u.email, u.name FROM $table m JOIN waitlist_users u ON m.user_id = u.id WHERE m.id = ?");
                $userStmt->execute([$mt5Id]);
                $userData = $userStmt->fetch(PDO::FETCH_ASSOC);
                ...
                $userStmt = $pdo->prepare("SELECT u.email, u.name, m.fail_reason FROM $table m JOIN waitlist_users u ON m.user_id = u.id WHERE m.id = ?");
                $userStmt->execute([$mt5Id]);
                $userData = $userStmt->fetch(PDO::FETCH_ASSOC);
            ...
            $details = ['mt5_id' => $mt5Id, 'table' => $table, 'new_status' => $newStatus];
            if ($newStatus === 'fail') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_test_type') {
        ...
        // Update test type
        $stmt = $pdo->prepare("UPDATE $table SET test_type = ? WHERE id = ?");
        $success = $stmt->execute([$newTestType, $mt5Id]);

        if ($success) {
            // Record audit
            $adminId = $_SESSION['admin_id'] ?? null;
            recordAdminAction($pdo, $adminId, 'update_test_type', $targetUserId, ['mt5_id' => $mt5Id, 'table' => $table, 'new_test_type' => $newTestType]);
    ...
    if (isset($_GET['export']) && $_GET['export'] === 'csv') {
        // Prepare query to fetch all under review MT5 details from both tables
        $export_query = "
            SELECT
                m.id,
                u.name,
                ...
                m.table_name
            FROM (
                SELECT id, user_id, username, password, server, instrument, status, submitted_at, 'mt5_details' as table_name
                FROM mt5_details
                WHERE status = 'under_review'
                UNION ALL
                SELECT id, user_id, username, password, server, instrument, status, submitted_at, 'mt5_details_second' as table_name
        ...
        header('Content-Disposition: attachment; filename=under_review_mt5_details_' . date('Y-m-d_H-i-s') . '.csv');
        ...
        // CSV headers
        fputcsv($output, ['ID', 'User Name', 'User Email', 'MT5 Username', 'MT5 Password', 'Server', 'Instrument', 'Status', 'Table', 'Submitted At']);
    ...
    // Get MT5 under review stats for each table separately
    $mt5Stats_mt1 = $pdo->query("SELECT COUNT(*) AS under_review_count FROM mt5_details WHERE status = 'under_review'")->fetch(PDO::FETCH_ASSOC);
    $mt5Stats_mt2 = $pdo->query("SELECT COUNT(*) AS under_review_count FROM mt5_details_second WHERE status = 'under_review'")->fetch(PDO::FETCH_ASSOC);

    // Get MT5 details with user info from both tables where status = 'under_review'
    $query = "
        SELECT
            m.*,
            u.name,
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Under Review MT5 Details</h1>
...
<div class="row mb-4">
    <div class="col-md-6">
        <div class="card text-white bg-info mb-3">
            <div class="card-body">
                <h5 class="card-title">Under Review - Test 1</h5>
                <h2><?php echo $mt5Stats_mt1['under_review_count'] ?? 0; ?></h2>
                <small>From table: mt5_details</small>
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card text-white bg-primary mb-3">
            <div class="card-body">
                <h5 class="card-title">Under Review - Test 2</h5>
                <h2><?php echo $mt5Stats_mt2['under_review_count'] ?? 0; ?></h2>
                <small>From table: mt5_details_second</small>
            </div>
        </div>
    </div>
</div>
            ...
            <table id="mt5Table" class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge bg-info" id="test-type-<?php echo $detail['table_name']; ?>_<?php echo $detail['id']; ?>"><?php echo htmlspecialchars($detail['test_type'] ?? '50:50 F4x'); ?></span>
                        </td>
                        <td>
                            <?php
                        ...
                        <td><?php echo htmlspecialchars($detail['table_name'] === 'mt5_details' ? 'Test 1 (mt5_details)' : 'Test 2 (mt5_details_second)'); ?></td>
                        <td><?php echo date('M d, Y H:i', strtotime($detail['submitted_at'])); ?></td>
                        <td>
                            <div class="dropdown">
                                <button class="btn btn-secondary btn-sm dropdown-toggle" type="button" id="dropdownMenuButton_<?php echo $detail['table_name']; ?>_<?php echo $detail['id']; ?>" data-bs-toggle="dropdown" aria-expanded="false">
                                    Actions
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton_<?php echo $detail['table_name']; ?>_<?php echo $detail['id']; ?>">
                                    ...
                                    <li><hr class="dropdown-divider"></li>
                                    <li><a class="dropdown-item" href="#" onclick="changeTestType(<?php echo $detail['id']; ?>, '<?php echo htmlspecialchars($detail['name']); ?>', '<?php echo htmlspecialchars($detail['test_type'] ?? '50:50 F4x'); ?>', '<?php echo $detail['table_name']; ?>')">
                                    ...
                                    <li><hr class="dropdown-divider"></li>
                                    <li><a class="dropdown-item text-success" href="#" onclick="updateStatus(<?php echo $detail['id']; ?>, 'pass', '<?php echo htmlspecialchars($detail['name']); ?>', '<?php echo $detail['table_name']; ?>')">
                                        <i class="bi bi-check-circle me-2"></i>Mark as Pass
                                    </a></li>
                                    <li><a class="dropdown-item text-primary" href="#" onclick="updateStatus(<?php echo $detail['id']; ?>, 'running', '<?php echo htmlspecialchars($detail['name']); ?>', '<?php echo $detail['table_name']; ?>', '<?php echo htmlspecialchars($detail['email']); ?>')">
                                        <i class="bi bi-play-circle me-2"></i>Mark as Running
                                    </a></li>
                                    <li><a class="dropdown-item text-danger" href="#" onclick="updateStatus(<?php echo $detail['id']; ?>, 'fail', '<?php echo htmlspecialchars($detail['name']); ?>', '<?php echo $detail['table_name']; ?>')">
                                        <i class="bi bi-x-circle me-2"></i>Mark as Fail
                                    </a></li>
                                    <li><a class="dropdown-item text-info" href="#" onclick="updateStatus(<?php echo $detail['id']; ?>, 'under_review', '<?php echo htmlspecialchars($detail['name']); ?>', '<?php echo $detail['table_name']; ?>')">
                                        <i class="bi bi-clock me-2"></i>Mark as Under Review
                                    </a></li>
                                    <li><a class="dropdown-item text-warning" href="#" onclick="updateStatus(<?php echo $detail['id']; ?>, 'pending', '<?php echo htmlspecialchars($detail['name']); ?>', '<?php echo $detail['table_name']; ?>')">
...
<script>
    $(document).ready(function() {
        var table = $('#mt5Table').DataTable({
            ...
            language: {
                search: "_INPUT_",
                searchPlaceholder: "Search Under Review MT5 details...",
            ...
            columnDefs: [
                {
                    targets: [11], // Actions column (now 11th due to Table column)
                    orderable: false
                }
            ],
    ...
    function updateStatus(mt5Id, newStatus, userName, tableName, userEmail = null) {
        if (newStatus === 'fail') {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Send AJAX request
                $.ajax({
                    url: 'under_review_mt5_details.php',
                    type: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            // Find the row containing this dropdown (table-aware id)
                            const dropdownButton = document.querySelector(`#dropdownMenuButton${tableName}_${mt5Id}`);
                            const row = dropdownButton ? dropdownButton.closest('tr') : null;
                            const statusCell = row ? row.querySelector('td:nth-child(9) .badge') : null; // Status is 9th column now
    ...
    $(document).ready(function() {
        $('#submitFail').on('click', function() {
            ...
            formData.append('table', window.currentTableName);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Update the status badge in the same row
                        const dropdownButton = document.querySelector(`#dropdownMenuButton${window.currentTableName}_${window.currentMt5Id}`);
                        const row = dropdownButton ? dropdownButton.closest('tr') : null;
                        const statusCell = row ? row.querySelector('td:nth-child(9) .badge') : null; // Status is 9th column
                        if (statusCell) {
    ...
    $(document).ready(function() {
        ...
        $('#submitPass').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Update the status badge in the same row (table-aware id)
                        const dropdownButton = document.querySelector(`#dropdownMenuButton${window.currentTableName}_${window.currentMt5Id}`);
                        const row = dropdownButton ? dropdownButton.closest('tr') : null;
                        const statusCell = row ? row.querySelector('td:nth-child(9) .badge') : null; // Status is 9th column
                        if (statusCell) {
    ...
    function changeTestType(mt5Id, userName, currentTestType, tableName) {
    ...
    $(document).ready(function() {
        $('#submitTestType').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Update the test type display in the same row (table-aware id)
                        const dropdownButton = document.querySelector(`#dropdownMenuButton${window.currentTableName}_${window.currentMt5Id}`);
                        const row = dropdownButton ? dropdownButton.closest('tr') : null;
                        const testTypeCell = row ? row.querySelector(`#test-type-${window.currentTableName}_${window.currentMt5Id}`) : null;
                        if (testTypeCell) {
